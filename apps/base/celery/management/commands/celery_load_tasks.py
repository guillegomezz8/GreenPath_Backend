...
class Command(BaseCommand):
    ...
    def handle(self, *args, **kwargs):
        logging.info("Iniciando carga de tareas desde celery_tasks.json")
        try:
            ...
            if not json_path.exists():
                ...
                logging.warning("Archivo celery_tasks.json no encontrado, omitiendo carga de tareas.")
                return
            ...
            for task_data in data.get("tareas", []):
                ...
                if schedule_type == "crontab":
                    ...
                    logging.info(f"[CRONTAB] Horario creado o encontrado: {schedule}")
                    ...
                    logging.info(f"[INTERVAL] Horario creado o encontrado: {schedule}")
 
                else:
                    logging.warning(f"Tarea '{task_data['name']}' ignorada: tipo de horario desconocido '{schedule_type}'")
                    continue
                ...
                if not PeriodicTask.objects.filter(name=task_data["name"]).exists():
                    ...
                    logging.info(f"Tarea '{task_data['name']}' creada en Celery Beat")
                else:
                    self.stdout.write(self.style.WARNING(f"La tarea '{task_data['name']}' ya existe."))
                    logging.warning(f"La tarea '{task_data['name']}' ya existe.")
            ...
            logging.error(f"Error al cargar tareas desde JSON: {e}")