...
| RF-060 | El sistema debe permitir descargar la factura PDF generandola bajo demanda con la venta y su foto fiscal asociada. |
...
| RF-060D | El sistema debe congelar los datos fiscales y bancarios del emisor al crear una venta, reutilizando snapshots iguales y creando uno nuevo si cambia la configuracion. |
...
| Codigo | Requisito |
| --- | --- |
| RF-086 | El PDF de factura debe incluir los datos fiscales congelados del emisor y los datos del comprador. |
| RF-087 | El PDF de factura debe incluir numero de factura, fecha, concepto, base, IVA y total. |
| RF-088 | El sistema debe permitir volver a descargar la factura generandola de nuevo sin modificar los datos fiscales historicos del emisor. |
...
| RF-104 | El sistema debe reconstruir el documento PDF bajo demanda usando la venta y su snapshot fiscal asociado cada vez que se descargue. |
